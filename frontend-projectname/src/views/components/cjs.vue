...
<template>
  <div
    class="home"
    style="position: relative; height: 100%; width: 100%; z-index: 0"
  >
    <side-bar
    ...
    <div style="flex-direction: column; flex: 1 1 auto; overflow: hidden">
      <search-bar
    ...
    >
      <el-form :model="node_form" label-width="120px">
    ...
    <el-dialog
      v-dialogDrag
      title="修改关系"
      :visible.sync="dialogRelation_Visible"
      :modal-append-to-body="false"
    >
      <el-form :model="relation_form" label-width="120px">
        <el-form-item label="关系">
          <el-input
            v-model="relation_form.data.name"
            autocomplete="off"
          ></el-input>
        </el-form-item>
    ...
    >
      <el-form :model="relation_form" label-width="120px">
        ...
        <el-form-item label="目标节点名称">
          <el-select v-model="target_name" filterable placeholder="请选择">
            <el-option
              v-for="item in getNodes()"
              :key="item.id"
              :label="item.id"
              :value="item.id"
            >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          @click="add_relation(source_name, target_name)"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      :modal-append-to-body="false"
    >
      <el-form :model="editNode_form" label-width="120px">
        <el-form-item label="节点大小">
          <el-slider
            style="margin-left: 70px"
            v-model="editNode_form.size"
          ></el-slider>
        </el-form-item>
        <el-form-item label="字体大小">
          <el-slider
            style="margin-left: 70px"
            v-model="editNode_form.fontSize"
          ></el-slider>
        </el-form-item>
        <el-form-item label="节点颜色">
          <el-color-picker
            v-model="editNode_form.color"
    ...
    <el-dialog
      v-dialogDrag
      title="展示效果调节"
      :visible.sync="editShowStyleVisible"
      :modal-append-to-body="false"
    >
      <el-form :model="editShowStyle_form" label-width="120px">
    ...
    >
      <div class="demo-image__lazy">
        <span style="font-size: 15px"
          >注：选中节点（关系）后右键鼠标进入辅助菜单，再次点击节点（关系）取消操作，如下图:</span
        >
        <br />
...
export default {
  ...
  methods: {
    ...
    getNodes() {
      let data = this.$cy.elements().map((x) => x.data());
      return data.filter((x) => !x.source);
    },
    ...
    handleOperation(key) {
      switch (key) {
        ...
        case 2:
          ...
          break;
        case 3:
          this.refresh({ name: "grid" });
          break;
        case 4:
          this.refresh({ name: "avsdf" });
          break;
        case 5:
          this.editNodeVisible = true;
          break;
        case 6:
          this.editShowStyleVisible = true;
          break;
        case 7:
          this.exportPng();
          break;
    ...
    question_answer2(source_n, target_r) {
      ...
      this.$cy.elements().map(function (ele) {
        if (
          ele.data()["source"] == source_n &&
          ele.data()["name"] == target_r
        ) {
          t = ele.data().target;
          alert(t);
        }
    ...
    search(events) {
      ...
      let lightEles = [];
      data.forEach((x) => {
        if (new RegExp(events.input).test(x.name)) {
          if (events.type == "2" && x.source) {
            lightEles.push(x.id);
          } else if (events.type == "1" && !x.source) {
            lightEles.push(x.id);
          }
        }
      });
      this.lightOn(lightEles);
    },
    ...
    add_son(ele) {
      this.$cy.startBatch();
      this.$cy.batch(() => {
        this.source_name = ele;
        this.relation_form.data.id = this.relation_index;
        this.relation_form.data.id = this.relation_index;
        // that.relation_form.data.source=ele;
        this.relation_index += 1;
        this.addSon_Visible = true;
      });
      this.$cy.endBatch();
    },
    ...
    add_relation(source_n, target_n) {
      ...
      this.relation_form.data.target = t;
      if (
        this.relation_form.data.source == "" ||
        this.relation_form.data.target == ""
      ) {
    ...
    modify() {
      ...
      const that = this;
      this.$cy.batch(() => {
        let relations = this.find_relations_modify(
          that.node_form.data.id,
          that.node_form.data.name
        );
    ...
    addNodeStyle() {
      ...
      this.$cy
        ...
        .css({
          "background-color": this.editNode_form.color,
          content: "data(name)",
          "border-color": this.editNode_form.color,
          "border-width": this.editNode_form.size + "px",
          "font-size": this.editNode_form.fontSize + "px",
          "pie-size": "100%",
        })
        .update();
      ...
      var t = {
        value: this.editNode_form.name,
        label: this.editNode_form.name,
      };
